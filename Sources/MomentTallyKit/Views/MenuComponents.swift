...
/// Tag Sets settings pane and the running-timer tag editor. On iOS the
/// system picker is a plain sheet, so no NSColorPanel caveats apply; the
/// Mac's panel-sweeping lives in MenuComponentsMac.swift.
package struct TagColorPicker: View {
    @Environment(AppModel.self) private var model
    package let key: String
    package let value: String
...
extension View {
    ...
    package func refreshesKeyViewLoop(on token: some Hashable) -> some View {
        background(KeyViewLoopRefresher(token: AnyHashable(token)))
    }
}
#else
extension View {
    /// iOS has no key-view loop; shared editors call this unconditionally,
    /// so it degrades to the identity.
    package func refreshesKeyViewLoop(on token: some Hashable) -> some View {
        self
    }
}
